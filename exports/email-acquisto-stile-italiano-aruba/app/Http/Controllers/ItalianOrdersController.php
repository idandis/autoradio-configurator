...
class ItalianOrdersController extends Controller
{
    ...
    public function testPurchaseEmail(Request $request, ItalianOrder $italianOrder): RedirectResponse
    {
        if (! ItalianPurchaseEmails::configured()) {
            throw ValidationException::withMessages(['email' => 'L’invio SMTP delle conferme non è configurato.']);
        }

        try {
            Mail::to($request->user()->email)->send(
                new ItalianPurchaseConfirmation($italianOrder->load('items'), preview: true)
            );
        } catch (\Throwable $exception) {
            report($exception);
            throw ValidationException::withMessages(['email' => 'Invio di prova non riuscito. Controlla il log del server.']);
        }

        return to_route('italian-orders.show', $italianOrder)
            ->with('status', 'Email di prova inviata a '.$request->user()->email.'.');
    }
