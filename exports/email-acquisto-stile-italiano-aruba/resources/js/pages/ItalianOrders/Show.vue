...
const sendTestEmail = () => testEmailForm.post(`/italian-orders/${props.order.id}/purchase-email/test`, { preserveScroll: true });
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            ...
            <aside class="space-y-6">
                <section class="space-y-3 rounded-xl border bg-card p-5">
                    ...
                    <button type="button" :disabled="testEmailForm.processing" class="w-full rounded-md border px-4 py-2 text-sm disabled:opacity-50" @click="sendTestEmail">{{ testEmailForm.processing ? 'Invio…' : 'Invia prova alla mia email' }}</button>
                    <p v-for="(error, key) in testEmailForm.errors" :key="key" role="alert" class="text-sm text-destructive">{{ error }}</p>
                </section>
