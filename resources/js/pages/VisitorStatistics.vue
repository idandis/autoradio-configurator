...
type ExtraEuVisitor = {
    ...
    is_bot: boolean; bot_reason: string | null; bot_blocked: boolean;
...
<template>
    ...
    <div class="flex flex-1 flex-col gap-6 p-4">
        ...
        <section class="overflow-hidden rounded-xl border border-amber-400/70 bg-card">
            <div class="border-b border-amber-300 bg-amber-50 px-5 py-4 dark:bg-amber-950/20">
                <h2 class="text-lg font-semibold">Bot rilevati <span class="ml-2 rounded-full bg-amber-100 px-2.5 py-1 text-sm text-amber-800 dark:bg-amber-950 dark:text-amber-300">{{ extraEuBotTotal }}</span></h2>
                <p class="mt-1 text-sm text-muted-foreground">Crawler e richieste automatiche separati dai visitatori. I bot sospetti vengono respinti; i motori di ricerca riconoscibili restano autorizzati.</p>
            </div>
            <div class="overflow-x-auto"><table class="w-full min-w-[1650px] text-sm"><thead class="border-b bg-muted/40 text-left text-xs uppercase text-muted-foreground"><tr><th v-for="head in ['Prima visita','Ultima visita','Accessi','Stato','Motivo','Paese','Regione','Città','Dispositivo','Lingua browser','Provenienza','Pagina','User agent']" :key="head" class="px-4 py-3">{{ head }}</th></tr></thead><tbody class="divide-y"><tr v-for="visitor in extraEuBots.data" :key="visitor.id" class="hover:bg-muted/20"><td class="whitespace-nowrap px-4 py-3">{{ dateTime(visitor.first_seen_at) }}</td><td class="whitespace-nowrap px-4 py-3">{{ dateTime(visitor.last_seen_at) }}</td><td class="px-4 py-3 font-semibold">{{ visitor.hits }}</td><td class="px-4 py-3"><span class="rounded-full px-2 py-1 text-xs font-semibold" :class="visitor.bot_blocked ? 'bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300'">{{ visitor.bot_blocked ? 'Respinto' : 'Autorizzato' }}</span></td><td class="max-w-64 px-4 py-3">{{ visitor.bot_reason||'Automatico' }}</td><td class="px-4 py-3 font-semibold">{{ visitor.country_code }}</td><td class="px-4 py-3">{{ visitor.region||'—' }}</td><td class="px-4 py-3">{{ visitor.city||'—' }}</td><td class="px-4 py-3 capitalize">{{ visitor.device_type||'—' }}</td><td class="max-w-52 truncate px-4 py-3" :title="visitor.browser_language||''">{{ visitor.browser_language||'—' }}</td><td class="max-w-60 truncate px-4 py-3" :title="visitor.referrer||''">{{ visitor.referrer||'Provenienza non comunicata' }}</td><td class="max-w-52 truncate px-4 py-3" :title="visitor.requested_path||''">{{ visitor.requested_path||'—' }}</td><td class="max-w-80 truncate px-4 py-3" :title="visitor.user_agent||''">{{ visitor.user_agent||'—' }}</td></tr><tr v-if="!extraEuBots.data.length"><td colspan="13" class="py-10 text-center text-muted-foreground">Nessun bot rilevato.</td></tr></tbody></table></div>
            <div v-if="extraEuBots.links.length > 3" class="flex flex-wrap items-center justify-between gap-3 border-t px-4 py-4"><p class="text-sm text-muted-foreground">{{ extraEuBots.from??0 }}–{{ extraEuBots.to??0 }} di {{ extraEuBots.total }}</p><nav class="flex flex-wrap gap-1"><template v-for="link in extraEuBots.links" :key="link.label"><Link v-if="link.url" :href="link.url" preserve-state class="rounded border px-3 py-2 text-sm" :class="link.active?'bg-primary text-primary-foreground':'hover:bg-accent'"><span v-html="link.label"></span></Link><span v-else class="rounded border px-3 py-2 text-sm opacity-40" v-html="link.label"></span></template></nav></div>
        </section>
    </div>
</template>
